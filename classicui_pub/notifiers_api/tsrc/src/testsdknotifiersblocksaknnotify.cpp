/*
* Copyright (c) 2002 - 2007 Nokia Corporation and/or its subsidiary(-ies).
* All rights reserved.
* This component and the accompanying materials are made available
* under the terms of "Eclipse Public License v1.0"
* which accompanies this distribution, and is available
* at the URL "http://www.eclipse.org/legal/epl-v10.html".
*
* Initial Contributors:
* Nokia Corporation - initial contribution.
*
* Contributors:
*
* Description:  Test AknNotify.h
*
*/

/*
 * Include files
 */
#include <aknnotify.h>
#include <aknglobalnote.h>
#include <aknsddata.h>
#include "testsdknotifiers.h"

// -----------------------------------------------------------------------------
// CTestSDKNotifiers::TestNotifiersNotifyDestructorL
// -----------------------------------------------------------------------------
//
TInt CTestSDKNotifiers::TestNotifiersNotifyDestructorL( CStifItemParser& /*aItem*/ )
    {
    CAknGlobalNote* note = CAknGlobalNote::NewL();
    CleanupStack::PushL( note );
    STIF_ASSERT_NOT_NULL( note );
    CleanupStack::PopAndDestroy( note );
        
    return KErrNone;
    }

// -----------------------------------------------------------------------------
// CTestSDKNotifiers::TestNotifiersNotifySetSecondaryDisplayDataL
// -----------------------------------------------------------------------------
//
TInt CTestSDKNotifiers::TestNotifiersNotifySetSecondaryDisplayDataL( CStifItemParser& /*aItem*/ )
    {
    CAknGlobalNote* note = CAknGlobalNote::NewL();
    CleanupStack::PushL( note );
    CAknSDData* sddata = new( ELeave ) CAknSDData;
    CleanupStack::PushL( sddata );
    note->SetSecondaryDisplayData( sddata );
    
    CleanupStack::Pop( sddata );
    CleanupStack::PopAndDestroy( note );
    return KErrNone;
    }


//End file
