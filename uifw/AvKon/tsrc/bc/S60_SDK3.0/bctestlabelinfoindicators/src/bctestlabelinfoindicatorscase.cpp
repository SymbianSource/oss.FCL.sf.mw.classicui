...
_LIT( KEFNLRsc, "Invoke CEikFileNameLabel's ConstructFromResourceL()" );
_LIT( KEFNLCount, "Invoke CEikFileNameLabel's CountComponentControls()" );
...
void CBCTestLabelInfoIndicatorsCase::TestFunctionsForFileNameLabelL()
    ...
    // test private API
    CCoeControl* fnlabl = new CEikFileNameLabel();
    CleanupStack::PushL( fnlabl ); 
    TResourceReader reader;
    
    iEikEnv->CreateResourceReaderLC( 
        reader, R_BCTESTLABELINFOINDICATORS_LABEL );
    fnlabl->ConstructFromResourceL( reader );
    AssertTrueL( ETrue, KEFNLRsc );  
    CleanupStack::PopAndDestroy(); //reader 
    
    TInt count = fnlabl->CountComponentControls();
    AssertTrueL( count>0, KEFNLCount );
    CleanupStack::PopAndDestroy( fnlabl );           
    }
